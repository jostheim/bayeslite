...
def bayesdb_has_table(bdb, name):
    ...
    import tables
    return bdb.hdf5_store.get_node('/', name) is not None
    # try:
    #     cursor.next()
...
def bayesdb_table_column_names(bdb, table):
    ...
    bayesdb_column_table = bdb.hdf_store.get_node('/', 'bayesdb_column')
    rows = bayesdb_column_table.read_where("bayesdb_column", "tabname = {0}".format(table))
    # sql = '''
...
def bayesdb_table_has_column(bdb, table, name):
    ...
    bayesdb_column_table = bdb.hdf_store.get_node('/', 'bayesdb_column')
    row = bayesdb_column_table.read_where("(tabname = {0}) & (name = {1})".format(table, name))
    return row[0]
...
def bayesdb_table_column_name(bdb, table, colno):
    ...
    #     return row[0]
    bayesdb_column_table = bdb.hdf_store.get_node('/', 'bayesdb_column')
    row = bayesdb_column_table.read_where("(tabname = {0}) & (colno = {1})".format(table, colno))
    return row[0]['name']
...
def bayesdb_table_guarantee_columns(bdb, table):
    ...
    """
    with bdb.savepoint():
        tbl = bdb.hdf_store[table]
        bayesdb_column_table = bdb.hdf_store.get_node('/', 'bayesdb_column')
        nrows = 0
        new_row = bayesdb_column_table.row
        for row in tbl:
            nrows += 1
            colno, name = row
            new_row['colno'] = colno
...
def bayesdb_has_generator_default(bdb, name):
    ...
    # TODO not so sure about the default stuff
    bayesdb_generator_table = bdb.hdf_store.get_node('/', 'bayesdb_generator')
    rows = bayesdb_generator_table.read_where("(name = {0}))".format(name))
    return 0 != len(rows)
    # # sql = '''
